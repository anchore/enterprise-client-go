...
// GetImageSbomNativeOpts Optional parameters for the method 'GetImageSbomNative'
type GetImageSbomNativeOpts struct {
    XAnchoreAccount optional.String
}
 ...
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param imageDigest
 * @param optional nil or *GetImageSbomNativeOpts - Optional Parameters:
 * @param "XAnchoreAccount" (optional.String) -  An account name to change the resource scope of the request to that account, if permissions allow (admin only)
@return *os.File
*/
func (a *ImagesApiService) GetImageSbomNative(ctx _context.Context, imageDigest string, localVarOptionals *GetImageSbomNativeOpts) (*os.File, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodGet
		localVarPostBody     interface{}
		localVarFormFileName string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/images/{imageDigest}/sboms/native"
	localVarPath = strings.Replace(localVarPath, "{"+"imageDigest"+"}", _neturl.QueryEscape(parameterToString(imageDigest, "")) , -1)
